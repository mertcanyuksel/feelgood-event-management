-- CRM_365'ten FEELGOOD'a GÜVENLI VERİ AKTARIMI
-- Foreign key constraint ihlallerini önler
...
-- 1.3 uzm_businesscard (TÜM KARTLARI AKTAR)
PRINT '  >> uzm_businesscard aktarılıyor (tüm kullanılan kartlar)...';

-- Önce event'lerde kullanılan TÜM business card ID'lerini topla
INSERT INTO uzm_businesscard (uzm_businesscardId, uzm_name, is_active)
SELECT DISTINCT
    bc.uzm_businesscardId,
    bc.uzm_name,
    1 AS is_active
FROM (
    -- BusinessCard1
    SELECT DISTINCT UE.uzm_BusinessCard1 AS bcId
    FROM CRM_365.dbo.uzm_event UE
    WHERE UE.uzm_BusinessCard1 IS NOT NULL
      AND UE.uzm_eventtypeid = 'C89A605F-7F52-F011-8BAA-005056A1F1F4'
      AND UE.statecode = 0

    UNION

    -- BusinessCard2
    SELECT DISTINCT UE.uzm_BusinessCard2
    FROM CRM_365.dbo.uzm_event UE
    WHERE UE.uzm_BusinessCard2 IS NOT NULL
      AND UE.uzm_eventtypeid = 'C89A605F-7F52-F011-8BAA-005056A1F1F4'
      AND UE.statecode = 0
    ...
    -- BusinessCard3
    SELECT DISTINCT UE.uzm_BusinessCard3
    FROM CRM_365.dbo.uzm_event UE
    WHERE UE.uzm_BusinessCard3 IS NOT NULL
      AND UE.uzm_eventtypeid = 'C89A605F-7F52-F011-8BAA-005056A1F1F4'
      AND UE.statecode = 0
    ...
    -- BusinessCard4
    SELECT DISTINCT UE.uzm_BusinessCard4
    FROM CRM_365.dbo.uzm_event UE
    WHERE UE.uzm_BusinessCard4 IS NOT NULL
      AND UE.uzm_eventtypeid = 'C89A605F-7F52-F011-8BAA-005056A1F1F4'
      AND UE.statecode = 0
    ...
    -- BusinessCard5
    SELECT DISTINCT UE.uzm_BusinessCard5
    FROM CRM_365.dbo.uzm_event UE
    WHERE UE.uzm_BusinessCard5 IS NOT NULL
      AND UE.uzm_eventtypeid = 'C89A605F-7F52-F011-8BAA-005056A1F1F4'
      AND UE.statecode = 0
) AS usedCards
INNER JOIN CRM_365.dbo.uzm_businesscard bc ON bc.uzm_businesscardId = usedCards.bcId
WHERE NOT EXISTS (
    SELECT 1 FROM uzm_businesscard WHERE uzm_businesscardId = bc.uzm_businesscardId
);
...
SELECT
    ...
    -- Business Cards - sadece FEELGOOD'da varsa ekle, yoksa NULL
    CASE
        WHEN EXISTS (SELECT 1 FROM uzm_businesscard WHERE uzm_businesscardId = UE.uzm_BusinessCard1)
        THEN UE.uzm_BusinessCard1
        ELSE NULL
    END AS uzm_BusinessCard1,

    CASE
        WHEN EXISTS (SELECT 1 FROM uzm_businesscard WHERE uzm_businesscardId = UE.uzm_BusinessCard2)
        THEN UE.uzm_BusinessCard2
        ELSE NULL
    END AS uzm_BusinessCard2,

    CASE
        WHEN EXISTS (SELECT 1 FROM uzm_businesscard WHERE uzm_businesscardId = UE.uzm_BusinessCard3)
        THEN UE.uzm_BusinessCard3
        ELSE NULL
    END AS uzm_BusinessCard3,

    CASE
        WHEN EXISTS (SELECT 1 FROM uzm_businesscard WHERE uzm_businesscardId = UE.uzm_BusinessCard4)
        THEN UE.uzm_BusinessCard4
        ELSE NULL
    END AS uzm_BusinessCard4,

    CASE
        WHEN EXISTS (SELECT 1 FROM uzm_businesscard WHERE uzm_businesscardId = UE.uzm_BusinessCard5)
        THEN UE.uzm_BusinessCard5
        ELSE NULL
    END AS uzm_BusinessCard5,
...
-- NULL business card kontrolü
DECLARE @nullCards INT = (
    SELECT COUNT(*)
    FROM uzm_event
    WHERE uzm_BusinessCard1 IS NULL
       OR uzm_BusinessCard2 IS NULL
       OR uzm_BusinessCard3 IS NULL
       OR uzm_BusinessCard4 IS NULL
       OR uzm_BusinessCard5 IS NULL
);

IF @nullCards > 0
BEGIN
    PRINT 'UYARI: ' + CAST(@nullCards AS VARCHAR) + ' kayıtta eksik business card var (CRM_365''te silinmiş olabilir)';
    PRINT '';
END
